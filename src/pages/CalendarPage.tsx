    ...
    <Modal open={open} onClose={handleClose} title="یادداشت جدید">
      <div className="flex flex-col gap-4">
        <textarea
          ...
          rows={5}
          className="w-full px-3 py-2.5 text-sm rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-400 resize-none"
        />
        <div className="flex gap-3">
          ...
          <button
            ...
            className="flex-1 py-2.5 rounded-xl bg-indigo-500 hover:bg-indigo-600 disabled:opacity-40 text-sm font-bold text-white transition-colors"
          >
            ذخیره
          </button>
        </div>
...
function EventModal({
  ...
  const FIELD = 'w-full px-3 py-2.5 text-sm rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-violet-400 transition-shadow';
    ...
    <div className="flex flex-col gap-4 pt-4 pb-6">
      ...
      <div className="flex items-center justify-between mt-1">
        <div className="flex items-center gap-2">
          ...
          {selectedIsToday && (
            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-indigo-500 text-white">امروز</span>
          )}
        ...
        <button
          ...
          className="flex items-center gap-1 text-xs font-bold px-3 py-1.5 rounded-xl bg-indigo-500 hover:bg-indigo-600 text-white transition-colors"
        >
      ...
      {/* Tasks section */}
      <section>
        <h3 className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-2">وظایف</h3>
        {dayTasks.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-gray-200 dark:border-gray-700 px-4 py-5 text-center">
            <p className="text-sm text-gray-400 dark:text-gray-500">وظیفه‌ای برای این روز ثبت نشده</p>
            <button
              onClick={() => setShowTaskForm(true)}
      ...
      <section>
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wide">رویدادها</h3>
          <button
        ...
        {dayEvents.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-gray-200 dark:border-gray-700 px-4 py-4 text-center">
            <p className="text-sm text-gray-400 dark:text-gray-500">رویدادی ثبت نشده</p>
          </div>
        ) : (
          <div className="flex flex-col gap-2">
            {dayEvents
      ...
      <section>
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wide">یادداشت‌ها</h3>
          <button
        ...
        {dayNotes.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-gray-200 dark:border-gray-700 px-4 py-4 text-center">
            <p className="text-sm text-gray-400 dark:text-gray-500">یادداشتی برای این روز وجود ندارد</p>
          </div>
        ) : (
          <div className="flex flex-col gap-2">
            {dayNotes.map(note => (
